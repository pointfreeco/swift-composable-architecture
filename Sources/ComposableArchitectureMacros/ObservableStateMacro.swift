...
extension ObservableStateMacro: ExtensionMacro {
  public static func expansion(
    ...
    let decl: DeclSyntax = """
        extension \(raw: type.trimmedDescription): \(raw: qualifiedConformanceName), \(raw: qualifiedOriginalConformanceName) {}
        """
...
public struct ObservationStateTrackedMacro: AccessorMacro {
  ...
  >(
    ...
    let setAccessor: AccessorDeclSyntax =
      """
      set {
        if newValue is any Observable, isIdentityEqual(newValue, _\(identifier)) {
