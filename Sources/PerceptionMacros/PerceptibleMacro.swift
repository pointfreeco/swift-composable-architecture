...
extension PerceptibleMacro: MemberMacro {
  ...
  >(
    ...
    declaration.addIfNeeded(PerceptibleMacro.withMutationFunction(perceptibleType), to: &declarations)
...
extension PerceptionTrackedMacro: PeerMacro {
  ...
  >(
    ...
    guard let property = declaration.as(VariableDeclSyntax.self),
          property.isValidForPerception else {
      return []
    }
    ...
    let storage = DeclSyntax(property.privatePrefixed("_", addingAttribute: PerceptibleMacro.ignoredAttribute))
    return [storage]
  }
}
