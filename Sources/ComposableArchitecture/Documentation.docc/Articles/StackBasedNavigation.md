...
Learn about stack-based navigation, that is navigation modeled with collections, including
how to model your domains, how to integrate features, how to test your features, and more.

## Overview

Stack-based navigation is the process of modeling navigation using collections of state. This 
...
``ReducerProtocol/forEach(_:action:destination:file:fileID:line:)`` operator, as well as a new
view ``NavigationStackStore`` that behaves like `NavigationStack` but tuned specifically with the 
...
And then we must make use of the ``ReducerProtocol/forEach(_:action:destination:file:fileID:line:)``
to integrate the domains of all the features that can be navigated to with the domain of the parent
feature:
...
The `NavigationPath` data type is a type-erased collection of data that is tuned specifically
for `NavigationStack`s. It allows you to maximally decouple features in the stack since you can
add any kind of data to a path:
