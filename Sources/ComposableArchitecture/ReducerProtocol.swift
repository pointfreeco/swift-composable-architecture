  ...
  ///
  /// > Important: ``reduce(into:action:)-4nzr2`` takes precedence over
  /// > ``body-swift.property-5mc0o``. If a reducer conformance implements both requirements, only
  /// > ``reduce(into:action:)-4nzr2`` will be called by the ``Store``. If your reducer assembles a
  /// > body from other reducers _and_ has additional business logic it needs to layer into the
  /// > system, introduce this logic into the body instead, either with ``Reduce``, or with a
  /// > separate, dedicated conformance.
