...
> Important: This performance tip is only applicable if you are still using ``ViewStore`` and 
``WithViewStore``, which are both soft-deprecated. The tip below may be useful, but we 
recommend migrating your app to the new observation tools when possible. See 
<doc:MigratingTo1.6> for more information.
