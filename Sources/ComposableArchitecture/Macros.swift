...
/// ```
///
/// This will allow you to use key path syntax for specifying case paths in various APIs in the
...
/// as well as allow you to use dot-chaining syntax for optionally extracting a case from the state.
/// This can be useful when using the view modifiers that come with the library that allow for
...
/// types that are inside other types with macros applied. So, for example, if you wanted to extend
/// a reducer's `State` with some extra functionality:
///
/// ```swift
/// @Reducer
/// struct Feature {
...
/// ```
///
/// …this unfortunately does not work. This is a [known
/// issue](https://github.com/apple/swift/issues/66450), and the only workaround is to either move
/// the extension to a separate file, or move the code from the extension to be directly inside
/// the `State` type.
///
