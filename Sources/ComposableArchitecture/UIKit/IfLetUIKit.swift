...
extension Store {
  ...
  ///
  /// If the store's state is non-`nil`, it shall safely unwrap the value and bundle it into a new
  ...
  /// to navigate to be held as an optional value in the parent, and when that value goes from
  /// `nil` to non-`nil`, or non-`nil` to `nil`, thou update the navigation stack accordingly:
  ///
  /// ```swift
  ...
  ///   goes from `nil` to non-`nil` and vice versa, so that the caller react to these changes.
