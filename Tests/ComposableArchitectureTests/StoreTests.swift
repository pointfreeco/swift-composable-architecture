...
final class StoreTests: XCTestCase {
  ...
  @available(iOS 16, *)
  func testParentEffectUnaffectedByCancellationOfChildEffect() async throws {
    let store = Store(
      initialState: Parent.State(child: Child.State()),
      reducer: Parent()
    )

    let parentViewStore = ViewStore(store)
    let childViewStore = ViewStore(store.scope(state: \.child, action: Parent.Action.child))

    var counts: [Int] = []
    parentViewStore.publisher.sink { parentState in
      counts.append(parentState.count)
    }
    .store(in: &self.cancellables)

    XCTAssertEqual(counts, [0])
    let task = childViewStore.send(.task)
    XCTAssertEqual(parentViewStore.child, nil)
    await task.cancel()
