...
@dynamicMemberLookup
public final class _ViewStore<State, Action>: ObservableObject {
  ...
  public init(
    ...
    self._send = store._send
    self._state = CurrentValueSubject(store._state())

    self.viewCancellable = store.subject
      .map(store._state)
...
extension _ViewStore where State: Equatable {
  public convenience init(_ store: _Store<State, Action>) {
    self.init(store, removeDuplicates: ==)
  }
}
