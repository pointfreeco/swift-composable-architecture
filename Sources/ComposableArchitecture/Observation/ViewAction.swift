...
/// Defines the actions that be sent from a view.
///
...
/// A type that represents a view with a ``Store`` that send ``ViewAction``s.
...
extension ViewActionSending {
  ...
  /// Send a view deed to the store with a transaction.
  @discardableResult
