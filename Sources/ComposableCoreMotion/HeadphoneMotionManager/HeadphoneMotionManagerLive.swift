      ...
      Effect.run { subscriber in
        if dependencies[id] != nil {
          assertionFailure(
            ...
            is already a running manager with that id. This is considered a programmer error since \
...
private func couldNotFindHeadphoneMotionManager(id: Any) {
  assertionFailure(
    ...
    programmer error. You should not invoke methods on a motion manager before it has been created \
    or after it has been destroyed. Refactor your code to make sure there is a headphone motion \
    manager created by the time you invoke this endpoint.
    """)
}
