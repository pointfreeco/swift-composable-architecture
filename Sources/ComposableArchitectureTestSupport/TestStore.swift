...
> {
  ...
  public struct Step {
    ...
    /// A step that describes an action sent to a store and asserts against how the store's state is
    /// expected to change.
    ///
    /// - Parameters:
    ...
    ///   - update: A function that describes how the test store's state is expected to change.
    /// - Returns: A step that describes an action received by an effect and asserts against how the
    ///   store's state is expected to change.
