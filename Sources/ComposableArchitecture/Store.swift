...
//  @_spi(Internals) public var effectCancellables: [UUID: AnyCancellable] = [:]
  ...
  public func send(
    ...
    while index < self.bufferedActions.endIndex {
      ...
      @Sendable
      func startAsyncWork() {
        guard effect.operations.filter({ $0.async != nil }).count > 0 else { return }
      ...
      for operation in effect.operations {
        ...
        let childContinuation = Send<Action>.Continuation {
