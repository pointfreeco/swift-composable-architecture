...
public final class Store<State, Action> {
  ...
  init(
    ...
    state: AnyPublisher<State, Never>,
  ...
  public func scope<LocalState, LocalAction>(
    ...
    Store<LocalState, LocalAction>(
      currentState: { toLocalState(self.currentState()) },
      state: self.state.map(toLocalState).eraseToAnyPublisher(),
  ...
  where P.Output == LocalState, P.Failure == Never {
    ...
    return toLocalState(self.state)
      .map { localState in
        Store<LocalState, LocalAction>(
          currentState: { localState },
          state: toLocalState(
            Just(self.currentState()).eraseToAnyPublisher()
          ).eraseToAnyPublisher(),
          send: { self.send(fromLocalAction($0)) }
        )
      }
...
//    action fromLocalAction: @escaping (LocalAction) -> Action
//  ) -> Store<LocalState, LocalAction> {
//    var localState = toLocalState(self.state.value)
//    let localStore = Store<LocalState, LocalAction>(
//      initialState: localState,
//      reducer: { _localState, localAction in
//        self.send(fromLocalAction(localAction))
...
//        localState = toLocalState(newValue)
