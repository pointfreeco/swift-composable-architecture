...
enum AlertState<Action> {
  ...
  struct Alert {
    ...
    func toSwiftUI(send: @escaping (Action) -> Void) -> SwiftUI.Alert {
...
extension View {
  func _alert<Action, Content: View>(
    item: Binding<AlertState<Action>>,
    send: @escaping (Action) -> Void,
    @ViewBuilder content: @escaping (AlertState<Action>.Alert) -> Content
    ...
    self.alert(
      item: Binding<AlertState<Action>.Alert?>(
        ...
        set: {
          if let alert = $0 {
            item.wrappedValue = .show(alert)
          } else {
            item.wrappedValue = .dismissed
          }
      }),
