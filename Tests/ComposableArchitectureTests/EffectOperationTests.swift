  ...
  class EffectOperationTests: BaseTCATestCase {
    func testMergeDiscardsNones() async {
      ...
      switch effect.operation {
      case let .run(_, send):
        await send(.init(isolation: #isolation, send: { XCTAssertEqual($0, 42) }))
      default:
        XCTFail()
      }
    ...
    func testMergeFuses() async {
      ...
      switch effect.operation {
      case let .run(_, send):
        await send(.init(isolation: #isolation) { values.append($0) })
      default:
        XCTFail()
      }
    ...
    func testConcatenateFuses() async {
      ...
      switch effect.operation {
      case let .run(_, send):
        await send(.init(isolation: #isolation, send: { values.append($0) }))
      default:
        XCTFail()
      }
