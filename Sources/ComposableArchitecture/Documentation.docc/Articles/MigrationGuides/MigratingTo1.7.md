...
still migrate while continuing to support older clients by defining a wrapper for the API that makes
it available on older platforms by calling down to the available 
`navigationDestination(isPresented:)` API:
...
helper for driving navigation off of a binding of data rather than just a simple boolean:
